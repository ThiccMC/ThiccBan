...
async function getList(page: number) {
    ...
    const _sql_data = await mySQL({
        q: 'SELECT data.id, data.uuid, data.reason, data.banned_by_name, data.time, user.name FROM '+ process.env.TABLE_PREFIX +'warnings AS data LEFT JOIN '+ process.env.TABLE_PREFIX +'history AS user ON data.uuid = user.uuid ORDER BY data.id DESC LIMIT ?,10', 
        v: [(_pageCurrent - 1) * 10]
    })
