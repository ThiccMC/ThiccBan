...
export async function getServerSideProps({ params }) {
  const result = await excuteQuery({
    query: 'SELECT COUNT(id) AS bancounts FROM `litebans_bans`',
    values: null,
  });
  var bc = result[0][0].bancounts;

  var pT = bc%10 != 0 ? (Math.floor(bc/10) + 1) : bc/10;
  var pC = parseInt(params.page) > 1 && parseInt(params.page) <= pT ? parseInt(params.page) : 1;

  var query = "SELECT id, uuid, reason, banned_by_uuid, banned_by_name, removed_by_name, removed_by_reason, time, until FROM `litebans_bans` ORDER BY `id` DESC LIMIT " + (pC - 1) * 10 + ",10";
...
export default function ListBans({ data, pC, pT }) {
    return (
    <div>
      <Head>
        <title>Bans | ThiccMC</title>
      </Head>
      <main>
        <Navbar></Navbar>
        ...
        <div className="card" style={{maxWidth: '800px', margin: '0 auto', borderWidth: '0px', borderRadius: '0px'}}>
          <div className="card-body">
            <h3 className="text-center card-title" style={{fontFamily: 'Segoe UI', marginBottom: '20px'}}>List for Bans</h3>
            <SearchBar></SearchBar>
                  ...
                  <tbody>
                    {
                      data.map((a)=>{
                        ...
                        return (
                          <Link key={"id-" + a.id} href={`/info/ban-${a.id}`}>
