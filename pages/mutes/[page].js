...
export async function getServerSideProps({ params }) {
  const result = await excuteQuery({
    query: 'SELECT COUNT(id) AS kickcounts FROM `litebans_mutes`',
    values: null,
  });
  ...
  var query = "SELECT id, uuid, reason, banned_by_uuid, banned_by_name, time FROM `litebans_mutes` ORDER BY `id` DESC LIMIT " + (pC - 1) * 10 + ",10";
      ...
      <main>
        ...
        <div className="card" style={{maxWidth: '800px', margin: '0 auto', borderWidth: '0px', borderRadius: '0px'}}>
          <div className="card-body">
            <h3 className="text-center card-title" style={{fontFamily: 'Segoe UI', marginBottom: '20px'}}>List for Mutes</h3>
            <SearchBar></SearchBar>
                  ...
                  <tbody>
                    {
                      data.map((a)=>{
                        ...
                        return (
                          <Link key={"id-" + a.id} href={`/info/mute-${a.id}`}>
